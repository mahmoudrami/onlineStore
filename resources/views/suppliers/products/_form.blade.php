...
<div class="mb-3 border p-2 rounded">
  ...
  <div class="mb-3">
    <div>
      ...
      <input type="checkbox" name="{{ $attribute->name }}" class="form-checkbox mx-3" id="CheckBox-{{ $attribute->id }}"
        value="{{ $attribute->id }}" @checked(old($attribute->name))>
    </div>
...
<div class="mb-3 border p-2 rounded">
  <div class="form-check mb-2">
    <input class="form-check-input" type="checkbox" id="check_{{ $slug }}" name="{{ $attribute->name }}"
      value="{{ $attribute->id }}" onchange="toggleAttributeTable('{{ $slug }}')" {{ $isChecked ? 'checked' : '' }}>
        ...
        @for ($i = 0; $i < $count; $i++) <tr>
          ...
          <td>
            <input type="text" name="attributeValue[{{ $attribute->name }}][{{ $locale }}][]" class="form-control"
              value="{{ $values[$locale][$i] ?? '' }}">
          </td>
          @endforeach
          <td>
            <button type="button" onclick="deleteRow(event)" class="btn btn-sm btn-danger">X</button>
          </td>
          </tr>
          @endfor
      </tbody>
